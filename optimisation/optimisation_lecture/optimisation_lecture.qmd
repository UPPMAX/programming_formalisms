...
Variance within programming langues is bigger 
than variance between languages @prechelt2000empirical

![](prechelt_fig_2.png)

> Figure 2 from @prechelt2000empirical
 ...
 * Hard to debug
 * Overhead is underestimated, e.g. 10x as much memory for 2x speed 
   (source: HPC cluster training, now [behind a login](https://wiki.hpc.rug.nl/peregrine/start) :-( ))

## Bad advice 4

'Optimize the function where you feel the performance problem is'

Developers -also very experienced developers- 
are known to have a bad intuition @sutter2004cpp
...
def create(): return list(range(0, 5))
def reverse(x): 
    x.reverse() 
    return x
